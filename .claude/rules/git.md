...
2. `gh issue view <番号> --repo mishi5/mirror_cc --comments` でコメントを確認し、他者が対応開始済みでないか確認する
   - 「対応開始」等のコメントがあれば、**重複を避けるため対応を取りやめ、ユーザにその旨を報告する**
   - 対応開始コメントがなければ、`gh issue comment <番号> --repo mishi5/mirror_cc --body "対応開始します"` を投稿してから次へ進む
...
5. TDD でテスト → 実装 → 全テストパス
   ...
   - **plan / spec ドキュメントには必ず対象 Issue の URL（`https://github.com/mishi5/mirror_cc/issues/<番号>`）を記載すること。** 複数 Issue にまたがる場合は全て列挙する。
...
```bash
gh issue comment <number> --repo mishi5/mirror_cc --body "..."
gh issue close <number> --repo mishi5/mirror_cc
```

コメント例:
```
## 対応内容
- `src/game/fighter.ts`: ヒットストン終了後にジャンプと空中移動を許可
- `tests/fighter.test.ts`: 関連テスト3件追加
...
4. **全 Issue 完了後**: まとめブランチから main 向けの PR を作成する
   - PR タイトルに含まれる Issue 番号を列挙する（例: `#10 #11 #12 bundle: ゲームプレイ改善 Phase2`）
   - ユーザに動作確認を促す
